...
public class ChercherVisiteur extends JPanel {
	...
	public ChercherVisiteur(FenPrincipale menu) {
					...
					try {
							...
							// Si il y a un resultat on l'affiche, sinon on affiche un message d'alert
								if(result.isEmpty()){
									...
									model.setRowCount(0);
									// Ajout des nouvelles ligne dans le tableau avec un boucle qui récupér l'objet visiteur
								...
								table.addMouseListener(new MouseAdapter() {
								    public void mousePressed(MouseEvent me) {
								        ...
								        if (me.getClickCount() == 2) {
								            Controller.id = result.get(row).getMat();
								            Controller.row = row;
