...
public class ChercherPraticien extends JPanel{
	
	private static final long serialVersionUID = 390223031573181631L;
	String columnName[] = {"Numéro","Nom","Prenom","Adresse","CP","Ville","Coefnotoriété","Code"}; 
	String[][] data = {};
		
	public ChercherPraticien(FenPrincipale menu) {
		...
		search_button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				DAOPraticien praticien = (DAOPraticien) DAOFactory.getDAOPraticien();
				
					try {
							// On recupére nos objet trouvé dans la bdd à l'aide d'un tableau*/
							ArrayList<Praticien> result = praticien.findP(search_field.getText());
							// Si il y a un resultat on l'affiche, sinon on affiche un message d'alert
								if(result.isEmpty()){
									...
									model.setRowCount(0);
									// Ajout des nouvelles ligne dans le tableau avec un boucle qui récupér l'objet praticien
									for(int i = 0; i < result.size(); i++){
										// On creer un objet[] et on lui met en parametre l'incrémentation pour pouvoir les mettre l'une
										// Apres l'autre, puis on récupére les différent champ
